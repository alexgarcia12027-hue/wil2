...
(function() {
  ...
  function setupErrorFiltering() {
    ...
    console.warn = function(...args) {
      // Filtrar mensajes específicos que no son críticos en producción
      const message = args.join(' ');
      if (message.includes('Heroicons') && message.includes('Path no encontrado')) {
        return; // Silenciar completamente
      }
      if (message.includes('[FallbackLoader]') || message.includes('[UnifiedFix]') || 
          message.includes('[ModuleFix]') || message.includes('[ViteFix]') ||
          message.includes('[CloudflareProduction]') || message.includes('[OfflineBundle]')) {
        return; // Reducir logs de sistemas internos
      }
      if (message.includes('React Router') || message.includes('relative route resolution') ||
          message.includes('Multiple GoTrueClient instances')) {
        return; // Silenciar warnings conocidos
      }
