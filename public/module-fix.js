    ...
    window.addEventListener('error', function(event) {
      ...
      if (event.filename && event.filename.includes('/node_modules/.vite/deps/')) {
        ...
        if (moduleNameMatch && moduleNameMatch[1]) {
          ...
          // Cargar desde CDN si está disponible
          const cdnBaseModule = Object.keys(SAFE_MODULE_URLS).find(base => moduleName.startsWith(base));
          if (cdnBaseModule) {
            console.log(`[ModuleFix] Cargando ${moduleName} desde fallback local...`);
            loadModuleFromURL(cdnBaseModule, SAFE_MODULE_URLS[cdnBaseModule]);
          }
        }
      }
    }, true);
