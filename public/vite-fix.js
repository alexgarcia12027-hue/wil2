...
(function() {
  ...
  if (window.WebSocket && !window.__VITE_FIX_STATE__.wsPatched) {
    ...
    class EnhancedWebSocket extends WebSocket {
      ...
      _handleOpen(event) {
        console.log(`[ViteFix] WebSocket (${this._id}): Conexión abierta`);
        this._reconnectAttempts = 0; // Resetear intentos cuando se conecta correctamente
      }
      
      // Manejador de evento 'close'
      _handleClose(event) {
        console.log(`[ViteFix] WebSocket (${this._id}): Conexión cerrada${event.wasClean ? ' limpiamente' : ''} (código: ${event.code})`);
        
        // Si la conexión se cerró correctamente o excedimos los intentos, no reconectar
        ...
        console.log(`[ViteFix] WebSocket (${this._id}): Intentando reconexión #${this._reconnectAttempts} en ${delay}ms`);
        
        // Programar intento de reconexión
        setTimeout(() => {
          // Verificar que todavía es relevante esta conexión
    ...
    setInterval(cleanupWebSockets, 30000);
  }
