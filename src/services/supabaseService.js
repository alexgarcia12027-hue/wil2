...
export const getSupabaseClient = () => {
  ...
  try {
    ...
    // Verificar configuración antes de crear cliente
    const supabaseUrl = import.meta.env.VITE_SUPABASE_URL || globalConfig.SUPABASE_URL;
    const supabaseKey = import.meta.env.VITE_SUPABASE_ANON_KEY || globalConfig.SUPABASE_ANON_KEY;

    if (!supabaseUrl || !supabaseKey || supabaseUrl.includes('your-project') || supabaseKey.includes('your-anon-key')) {
      console.warn('Configuración de Supabase no válida, usando cliente de fallback');
      return {
  ...
  try {
    ...
    if (client && client.auth && typeof client.auth.getSession === 'function') {
      client.auth.getSession().then(({ data, error }) => {
        ...
        console.warn('Error en verificación de sesión Supabase:', err);
      });
    } else {
      console.warn('Cliente Supabase no disponible, usando modo fallback');
    }
  } catch (e) {
